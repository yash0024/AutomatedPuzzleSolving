...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self.is_empty():
            ...
            return self._root
        elif self._root not in (OP_ADD, OP_MULTIPLY):
    ...
    def __eq__(self, other: ExprTree) -> bool:
        ...
        if self._root != other._root:
            ...
            for k in range(len(self._subtrees)):
                if self._subtrees[k] != other._subtrees[k]:
                    return False
            return True
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        if self.is_empty():
            ...
            a = self._root
            if isinstance(a, str) and a not in (OP_ADD, OP_MULTIPLY):
                lookup[a] = 0
            for subtree in self._subtrees:
